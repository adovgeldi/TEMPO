...
class MovingAvgModel(BaseModel):
    ...
    This model forecasts future values by averaging the most recent `n_days` 
    ...
    Attributes:
        ...
        n_days (int): The number of days over which to calculate the moving average.
        y_vals (List[float]): A list containing the last `n_days` values of the 
    ...
    def predict(self, test_data: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            for _ in range(len(test)):
                lookback_period = np.min([len(y_vals_w_preds),self.n_days])
