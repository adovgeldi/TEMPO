...
class LightGBMModel(TreeBasedModel):
    """
    A time series forecasting model using the LightGBM algorithm.

    This class extends the `TreeBasedModel` base class and provides functionality 
    for training and predicting time series data using lagged and rolling window features.
    ...
    def __init__(self, 
        ...
        """
        Initializes the LightGBMModel with the specified configurations.

        Parameters:
            date_col (str): The name of the column containing date information.
            target_y (str): The name of the target variable to be forecasted.
            model_params (Dict[str, Any], optional): Hyperparameters for the LightGBM model.
        """
        ...
        self.train = None
        self.lags = None    # will be set in fit with the other params
        self.windows = None # will be set in fit with the other params
        logger.debug(f"Initialized LightGBMModel with target_y: '{target_y}' and date_col: '{date_col}'")
    ...
    def fit(self, 
        ...
        """
        Trains the LightGBM model on the given training data.
        ...
        #       To get around this, we can also pass in stringified lists, like "[1, 2, 3]"
